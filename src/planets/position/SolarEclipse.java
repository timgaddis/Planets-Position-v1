...
public class SolarEclipse extends Activity {
	...
	private Calendar firstEcl, lastEcl;
	...
	public native double[] solarData(double d2, double[] loc, double press,
			double temp);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		firstEcl = Calendar.getInstance();

		computeEclipses(firstEcl, 0, false);
	...
	private void computeEclipses(Calendar startTime, int backward, boolean local) {
		// compute next or previous 10 eclipses and save them to the DB
		double endDate;
		for (int i = 0; i < 10; i++) {

			// planetDbHelper.updateSolar(rowID, localType, globalType, local,
			// localMaxTime, localFirstTime, localSecondTime, localThirdTime,
			// localFourthTime, diaRatio, fracCover, sunAz, sunAlt, localMag,
			// sarosNum, sarosMemNum, moonAz, moonAlt, globalMaxTime,
			// globalBeginTime, globalEndTime, globalTotBegin, globalTotEnd,
			// globalCenterBegin, globalCenterEnd);

			// endDate = max eclipse time
		}
		// lastEcl = endDate;
	}
}
